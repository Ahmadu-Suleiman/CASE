...
class _EditCaseState extends State<EditCase> {
  ...
  Widget build(BuildContext context) {
    ...
    final String uidCase = ModalRoute.of(context)!.settings.arguments as String;
    return FutureBuilder(
        future: DatabaseCase.getCaseRecordAndThumbnails(uidCase),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const Loading();
          } else if (snapshot.hasData) {
            CaseRecordAndThumbnails caseRecordAndThumbnails =
                snapshot.data as CaseRecordAndThumbnails;
            caseRecord = caseRecordAndThumbnails.caseRecord;
            List<Uint8List?> thumbnails = caseRecordAndThumbnails.thumbnails;
            ...
            videos = caseRecord.videos
                ...
                .entries
                .map(
                    (entry) => Video(XFile(entry.value), thumbnails[entry.key]))
                .toList();
